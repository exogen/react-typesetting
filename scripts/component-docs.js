  ...
  for (const filename in components) {
    ...
    if (propNames.length) {
      ...
      propNames.forEach(name => {
        ...
        console.log(`<tr>`)
        console.log(
          `<td valign="top" rowspan="${extraRows.length + 1}">${required(name)}</td>`
        )
        console.log(`<td valign="top" colspan="2">${type}</td>`)
        console.log(
          `<td valign="top" align="right" rowspan="${extraRows.length + 1}">${renderValue(prop.defaultValue)}</td>`
        )
        if (prop.description) {
          console.log(
          ...
          console.log(
            `<td valign="top" valign="top" rowspan="${extraRows.length + 1}"></td>`
          )
        }
        console.log('</tr>')
...
function renderType (value, extraRows = [], depth = 0, topLevel = false) {
  ...
  if (value.name === 'arrayOf') {
    return `Array of… <br>\n${indent}&nbsp;&nbsp;${renderType(value.value, extraRows, depth + 1)}`
  }
  if (value.name === 'shape') {
